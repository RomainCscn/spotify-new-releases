...
import { ArtistAndLastAlbum } from "./types.ts";
...
const formatArtist = ({ image, name, url, lastAlbum }: ArtistAndLastAlbum) =>
  `
  <div style="background: #003533; border-radius: 16px; padding: 12px; margin-bottom: 24px; box-shadow: 0 12px 16px -15px rgba(0,0,0,0.3);">
    <a href="${url}" style="display: flex; align-items: center; margin-bottom: 16px; color: #abd1c6; text-decoration: none;">
      <img src="${image}" style="margin-right: 12px; border-radius: 100%" width="64px" height="64px"/>
      <div style="font-size: 1.25rem;">${name}</div>
    </a>
...
export const formatMail = (artists: ArtistAndLastAlbum[]) =>
  `<body style="background: #004643; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol';"">
    ${
    artists.map((artist: ArtistAndLastAlbum) => formatArtist(artist)).join("")
  }
